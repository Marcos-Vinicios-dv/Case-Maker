...
const Perfil = () => {
  ...
  function alternarSection(section) {
    ...
    if(section === "Login")
    setSection({ cadastrar:false, login: true })
  }
  return (
    <Container>
      <ToggleSection cadastrar={section.cadastrar} login={section.login}>  
        <div>
          <strong>Customize sua <span>MÁQUINA</span>!</strong>
          <button type="button" onClick={() => alternarSection("Cadastrar")}>Cadastre-se</button>
        </div>  
        <img src={Engrenagem} alt="" width="450px"/>               
        <div>
          <strong>Já possui uma conta? Basta<span> Logar</span></strong>
          <button type="button" onClick={() => alternarSection("Login")}>Login</button>
        </div>                 
      </ToggleSection>
